...
# src/host and src/esp are the two platform implementations of
# platform/http_client.h: each only ever compiles on its own platform, and
# neither guards its includes, so each belongs to exactly one pass.
CHECK_BOTH = ["src", "include"]
CHECK_HOST_ONLY = ["host_examples", "src/host"]
CHECK_ESP_ONLY = ["examples", "src/esp"]
