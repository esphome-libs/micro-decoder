...
# Find all source files, excluding build/ and esp/ directories
# src/esp/ contains ESP-IDF platform code that requires ESP-IDF headers unavailable on the host
SOURCES=$(find "$ROOT_DIR/src" "$ROOT_DIR/host_examples" \
    \( -path '*/build' -o -path '*/esp' \) -prune -o \
